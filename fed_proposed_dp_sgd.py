...
def prox_term(shared: SharedMelEncoder, ref_sd: Dict[str, torch.Tensor]) -> torch.Tensor:
    """FedProx penalty computed from live parameters so autograd can differentiate it."""
    reg = None
    for name, param in shared.named_parameters():
        if name not in ref_sd:
            continue
        ref = ref_sd[name].to(device=param.device, dtype=param.dtype)
        term = (param - ref).pow(2).sum()
        reg = term if reg is None else reg + term

    if reg is None:
        first_param = next(shared.parameters(), None)
        if first_param is None:
            return torch.zeros((), device=next(iter(ref_sd.values())).device)
...
def require_opacus_for_dp():
    if RDPAccountant is None or DPDataLoader is None:
        raise RuntimeError(
            "DP mode requires Opacus. Install it with: pip install 'opacus>=1.5'"
        )


def make_dp_train_loader(base_loader: DataLoader) -> DataLoader:
    """Use Poisson sampling required by the sampled-Gaussian RDP accountant."""
    require_opacus_for_dp()
    return DPDataLoader.from_data_loader(base_loader, distributed=False)


def _dp_record_loss(model, x, length, target, class_w, gamma, smoothing):
    logits = model(x, length)
    ce = F.cross_entropy(
        logits, target, weight=class_w,
...
def train_pd_local_dp(
    ...
    cid: str,
    perf_hist: Dict[str, List[Dict]],
    accountant,
    noise_multiplier: float,
    clip_norm: float,
    sample_rate: float,
    use_focal: bool = True,
    ref_shared_sd: Optional[Dict[str, torch.Tensor]] = None,
    mu_prox: float = 0.0,
    update_shared: bool = True,
    update_private: bool = True,
    base_smoothing: float = 0.03,
    l2sp_init_private: Optional[Dict[str, torch.Tensor]] = None,
    lambda_l2sp: float = 0.0,
) -> Tuple[PDClientModel, float, int]:
    """Exact record-level DP-SGD compatible with the packed bidirectional LSTM.

    Each record gradient is computed separately, clipped using a single flat L2 norm,
    averaged, and perturbed with Gaussian noise. FedProx/L2-SP gradients are added
    afterwards because they are data-independent conditional on previous DP outputs.
    """
    if noise_multiplier <= 0 or clip_norm <= 0:
        raise ValueError("noise_multiplier and clip_norm must be positive")
    if not (0.0 < sample_rate <= 1.0):
        raise ValueError(f"Invalid sample_rate={sample_rate}")

    model.to(device).train()
    class_w = class_w.to(device)
    gamma = compute_gamma_for_client(
        cid, perf_hist, cfg.f1_ref, cfg.max_gamma
    ) if use_focal else 0.0

    shared = list(model.shared_mel.parameters()) if update_shared else []
    private = (list(model.adapter.parameters()) + list(model.head.parameters())) if update_private else []
    params = [p for p in shared + private if p.requires_grad]
    groups = []
    if shared:
        groups.append({"params": shared, "lr": lr_shared})
    if private:
        groups.append({"params": private, "lr": lr_private})
    opt = torch.optim.AdamW(groups, weight_decay=cfg.weight_decay)

    losses, steps = [], 0
    for _ in range(epochs):
        for X, L, y, _ in dl:
            if y.numel() == 0:  # possible with Poisson sampling
                continue
            X, L, y = X.to(device), L.to(device), y.to(device)
            bsz = int(y.numel())
            ...
            for i in range(bsz):
                li = _dp_record_loss(
                    model, X[i:i+1], L[i:i+1], y[i:i+1],
                    class_w, gamma, base_smoothing
                )
                gs = torch.autograd.grad(li, params, allow_unused=True)
                norm2 = torch.zeros((), device=device)
                for g in gs:
                    if g is not None:
                        norm2 += g.detach().pow(2).sum()
                coef = min(1.0, float(clip_norm / (torch.sqrt(norm2 + 1e-12).item())))
                for j, g in enumerate(gs):
                    if g is not None:
                        sums[j].add_(g.detach(), alpha=coef)
                batch_losses.append(float(li.detach().item()))

            opt.zero_grad(set_to_none=True)
            noise_std = float(noise_multiplier * clip_norm / bsz)
            for p, s in zip(params, sums):
                p.grad = s.div(float(bsz)).add_(torch.randn_like(s) * noise_std)

            reg = None
            if mu_prox > 0 and ref_shared_sd is not None and update_shared:
                reg = mu_prox * prox_term(model.shared_mel, ref_shared_sd)
            if lambda_l2sp > 0 and l2sp_init_private is not None and update_private:
                q = lambda_l2sp * l2sp_private(model, l2sp_init_private, device)
                reg = q if reg is None else reg + q
            reg_value = 0.0
            if reg is not None and reg.requires_grad:
                rgs = torch.autograd.grad(reg, params, allow_unused=True)
                for p, rg in zip(params, rgs):
                    if rg is not None:
                        p.grad.add_(rg.detach())
                reg_value = float(reg.detach().item())
            ...
            steps += 1
            losses.append(float(np.mean(batch_losses)) + reg_value)

    return model, float(np.mean(losses)) if losses else 0.0, steps
...
def main():
    ...
    ap.add_argument("--dp", action="store_true",
                    help="Enable record-level client-side DP-SGD.")
    ap.add_argument("--noise_multiplier", type=float, default=1.0)
    ap.add_argument("--dp_clip_norm", type=float, default=1.0)
    ap.add_argument("--delta", type=float, default=1e-5)
    args = ap.parse_args()
    if args.dp:
        require_opacus_for_dp()
        if args.noise_multiplier <= 0:
            ap.error("--noise_multiplier must be > 0")
        if args.dp_clip_norm <= 0:
            ap.error("--dp_clip_norm must be > 0")
    ...
    with open(os.path.join(args.outdir, "config.json"), "w", encoding="utf-8") as f:
        ...
        run_cfg.update({
            "dp_privacy_unit": "record" if args.dp else None,
            "dp_accountant": "RDPAccountant" if args.dp else None,
            "dp_sampling": "Poisson" if args.dp else None,
            "dp_personalization_accounted": False if args.dp else None,
        })
    ...
    dp_sample_rates = {}
    dp_accountants = {}
    dp_steps = {cid: 0 for cid in pd_clients}
    for cid in pd_clients:
        if args.dp:
            dp_loaders[cid] = make_dp_train_loader(pd_pack[cid]["dl_tr"])
            dp_sample_rates[cid] = float(dp_loaders[cid].sample_rate)
            dp_accountants[cid] = RDPAccountant()
        else:
            dp_loaders[cid] = pd_pack[cid]["dl_tr"]
            dp_sample_rates[cid] = float(cfg.batch_size / max(dataset_sizes[cid], 1))
    ...
    for rd in range(1, int(args.rounds) + 1):
        ...
        for cid in pd_clients:
            if args.dp:
                models[cid], train_loss, used_steps = train_pd_local_dp(
                    models[cid], dp_loaders[cid], device, epochs_local[cid],
                    lr_shared_pd, lr_private, class_w[cid], cid, perf_hist,
                    accountant=dp_accountants[cid],
                    noise_multiplier=float(args.noise_multiplier),
                    clip_norm=float(args.dp_clip_norm),
                    sample_rate=float(dp_sample_rates[cid]),
                    use_focal=True, ref_shared_sd=ref_shared_sd,
                    mu_prox=cfg.mu_prox, update_shared=True,
                    update_private=True, base_smoothing=cfg.base_smoothing
                )
                dp_steps[cid] += int(used_steps)
            else:
                models[cid], train_loss = train_pd_local(
                    models[cid], pd_pack[cid]["dl_tr"], device,
                    epochs_local[cid], lr_shared_pd, lr_private,
                    class_w[cid], cid, perf_hist, True,
                    ref_shared_sd, cfg.mu_prox, True, True,
                    cfg.base_smoothing, cfg.max_grad_norm
                )
            ...
            if args.dp:
                row.update({
                    "dp_epsilon": float(dp_accountants[cid].get_epsilon(float(args.delta))),
                    "dp_delta": float(args.delta),
                    "dp_noise_multiplier": float(args.noise_multiplier),
                    "dp_clip_norm": float(args.dp_clip_norm),
                    "dp_sample_rate": float(dp_sample_rates[cid]),
                    "dp_steps": int(dp_steps[cid]),
                })
    ...
    if args.dp:
        ...
        for cid in pd_clients:
            privacy_rows.append({
                ...
                "accountant": "RDP",
                "sampling": "Poisson",
                "epsilon": float(dp_accountants[cid].get_epsilon(float(args.delta))),
                "delta": float(args.delta),
                "noise_multiplier": float(args.noise_multiplier),
                "clip_norm": float(args.dp_clip_norm),
                "sample_rate": float(dp_sample_rates[cid]),
                "steps": int(dp_steps[cid]),
                "training_records": int(dataset_sizes[cid]),
            })
        worst_eps = max(r["epsilon"] for r in privacy_rows)
        for r in privacy_rows:
            r["worst_client_epsilon"] = float(worst_eps)
        pd.DataFrame(privacy_rows).to_csv(
            os.path.join(args.outdir, "privacy_summary.csv"), index=False
        )
        with open(os.path.join(args.outdir, "privacy_summary.json"), "w", encoding="utf-8") as f:
            json.dump({
                "privacy_unit": "record",
                "accountant": "RDPAccountant",
                "sampling": "Poisson",
                "delta": float(args.delta),
                "worst_client_epsilon": float(worst_eps),
                "clients": privacy_rows,
                "note": "The final personalization stage is local-only and is not included in the server-facing privacy expenditure."
            }, f, indent=2)
        print(f"\n[DP] worst-client epsilon={worst_eps:.4f}, delta={args.delta:g}")
